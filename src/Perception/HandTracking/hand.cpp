...
int Hand::load(int _target_size, const float* _mean_vals, const float* _norm_vals, bool use_gpu, int num_threads)
{
    ...
    // char parampath[256];
    // char modelpath[256];
    // sprintf(parampath, "%s/%s-op.param", model_dir, modeltype);
    // sprintf(modelpath, "%s/%s-op.bin", model_dir, modeltype);

    // blazepalm_net.load_param(parampath);
    // blazepalm_net.load_model(modelpath);
    //
