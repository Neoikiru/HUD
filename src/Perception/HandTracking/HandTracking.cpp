...
glm::vec3 MapCameraPixelToLocal3D(const cv::Point2f& pixel, float depthZ) {
    const float cam_fx = 470.326f;
    const float cam_fy = 462.444f;
    const float cam_cx = 307.401f;
    const float cam_cy = 232.849f;
...
HandTracker::HandTracker(const std::shared_ptr<Core::SharedState>& state) : m_state(state) {}
...
void HandTracker::WorkerLoop() {
    ...
    while (m_running) {
        ...
        if (frame) {
            ...
            cv::Mat cv_frame(frame->height, frame->width, CV_8UC3, frame->data->data(), frame->stride);
