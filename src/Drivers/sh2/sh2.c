...
static int opProcess(sh2_t *pSh2, const sh2_Op_t *pOp) {
    ...
    while ((pSh2->pOp != 0) && ((pOp->timeout_us == 0) || ((now_us - start_us) < pOp->timeout_us))) {
...
static int sendExecutable(sh2_t *pSh2, uint8_t cmd) { return shtp_send(pSh2->pShtp, pSh2->executableChan, &cmd, 1); }
...
static void getFrsRx(sh2_t *pSh2, const uint8_t *payload, uint16_t len) {
    ...
    if ((status == FRS_READ_STATUS_UNRECOGNIZED_FRS_TYPE) || (status == FRS_READ_STATUS_BUSY) ||
        (status == FRS_READ_STATUS_OFFSET_OUT_OF_RANGE) || (status == FRS_READ_STATUS_DEVICE_ERROR)) {
        // Operation failed
    ...
    if ((*(pSh2->opData.getFrs.pWords) == 0) || (offset <= *(pSh2->opData.getFrs.pWords))) {
    ...
    if ((FRS_READ_DATALEN(resp->len_status) == 2) &&
        ((*(pSh2->opData.getFrs.pWords) == 0) || (offset <= *(pSh2->opData.getFrs.pWords)))) {
        pSh2->opData.getFrs.pData[offset + 1] = resp->data1;
        pSh2->opData.getFrs.nextOffset = offset + 2;
    }

    // If read is done, complete the operation
    if ((status == FRS_READ_STATUS_READ_RECORD_COMPLETED) || (status == FRS_READ_STATUS_READ_BLOCK_COMPLETED) ||
...
static void setFrsRx(sh2_t *pSh2, const uint8_t *payload, uint16_t len) {
    ...
    if (sendMoreData && (pSh2->opData.setFrs.offset < pSh2->opData.setFrs.words)) {
...
static int getOscTypeStart(sh2_t *pSh2) { return sendCmd0(pSh2, SH2_CMD_GET_OSC_TYPE); }
...
static int finishCalStart(sh2_t *pSh2) { return sendCmd1(pSh2, SH2_CMD_CAL, SH2_CAL_FINISH); }
